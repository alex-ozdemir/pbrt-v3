...
string LambdaMaster::getSummary() {
    ...
    for ( const auto & treeletEntry : workerStats.objectStats ) {
        if ( treeletEntry.first.type == ObjectType::Treelet ) {
            oss << treeletEntry.first.id << "\t"
                << treeletEntry.second.processedRays << endl;
        }
    }
